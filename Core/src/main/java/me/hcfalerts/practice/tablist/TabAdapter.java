...
import me.hcfalerts.practice.tablist.impl.GhostlyAdapter;
import me.hcfalerts.practice.tablist.impl.TabListCommons;
...
import me.hcfalerts.practice.tablist.impl.utils.TabColumn;
...
public class TabAdapter implements GhostlyAdapter {
    ...
    public List<String> getFooter(Player player) {
        ...
        String Playing = PlaceholderAPI.setPlaceholders(player, "%bungee_Practice%");
        footer.replaceAll(s -> s
                .replace("{player}", player.getName())
                .replace("{color}", profile.getColor())
                .replace("{clan}", profile.getClan() == null ? "" : profile.getClan().getName())
                .replace("{rank}", HCFPractice.get().getRankManager().getRank().getName(player.getUniqueId()))
    ...
    public List<String> getHeader(Player player) {
        ...
        Profile profile = Profile.get(player.getUniqueId());
        String Global = PlaceholderAPI.setPlaceholders(player, "&f%bungee_total%/3,500");
        header.replaceAll(s -> s
                ...
                .replace("{rank}", HCFPractice.get().getRankManager().getRank().getName(player.getUniqueId()))
                .replace("{tps}", TPSUtil.getTPS())
                .replace("{global}", CC.translate(Global))
                .replace("{ping}", String.valueOf(BukkitReflection.getPing(player))));
    ...
    public Set<BufferedTabObject> getSlots(Player player) {
        ...
        if (profile.getTabType() == TabType.MODERN) {
            for (int i = 1; i < 21; i++) {
        ...
        else if (profile.getTabType() == TabType.VANILLA) {
